...
# Monorepo root (sobrecito): ops/scripts -> sobrecito
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# Cargo workspace root (the ZK package) holds Cargo.toml + target/
ROOT_DIR="$REPO_ROOT/packages/zk"
...
if [[ -n "$VK_FILE" || -n "$VK_JSON" ]]; then
  if [[ -n "$VK_FILE" ]]; then
    ...
    "$SCRIPT_DIR/build-verifier-with-vk.sh" "$VK_FILE" --out-dir "$WASM_DIR"
  else
    TMP_VK="$(mktemp --suffix=.json)"
    printf '%s' "$VK_JSON" > "$TMP_VK"
    "$SCRIPT_DIR/build-verifier-with-vk.sh" "$TMP_VK" --out-dir "$WASM_DIR"
    rm -f "$TMP_VK"
  fi
fi
...
DEPLOYMENTS_DIR="$REPO_ROOT/ops/deployments/$NETWORK"
