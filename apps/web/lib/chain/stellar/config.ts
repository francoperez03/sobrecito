...
export function stellarConfig(): ChainConfig {
  ...
  // poolId points at the UltraHonk noir_pool. The legacy Groth16 stack (pools[],
  // asp_*, verifier) was purged in the full-Noir migration; the ASP allowlist
  // proofs were dropped in D2 (sobre_slim has no asp_* public inputs).
  const d = deployments as typeof deployments & {
    noir_pool: string
    noir_pool_token: string
  ...
  cached = {
    ...
    baseFee: BASE_FEE,
    poolId: d.noir_pool,
    usdcId: d.noir_pool_token,
    // ASP allowlist dropped in D2 — kept empty for ChainConfig shape stability.
    aspMembershipId: '',
    aspNonMembershipId: '',
