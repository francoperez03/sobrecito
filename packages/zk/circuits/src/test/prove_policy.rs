...
mod tests {
    ...
    /// Sobre payroll batch: one employer input holding the total `T`, split into
    /// eight employee outputs that sum to `T`. Exercises the conservation
    /// constraint `sumIns + publicAmount === sumOuts` for the 1-to-8 shape
    /// (NOTE-02 / NOTE-05). `publicAmount = 0`: fully shielded batch.
    #[test]
    #[ignore]
    fn test_tx_1in_8out_payroll() -> Result<()> {
        let (wasm, r1cs) = load_artifacts("policy_tx_1_8")?;

        // Eight employee salaries that sum to the employer's total note.
...
#[test]
#[ignore]
fn print_employer_mem_leaf() {
    use crate::test::utils::{general::{poseidon2_hash2, scalar_to_bigint}, keypair::derive_public_key};
    use zkhash::{ark_ff::Zero, fields::bn256::FpBN256 as Scalar};
    let priv_key = Scalar::from(424242u64);
    let pub_key = derive_public_key(priv_key);
    let mem_leaf = poseidon2_hash2(pub_key, Scalar::zero(), Some(Scalar::from(1u64)));
    println!("pub_key: {}", scalar_to_bigint(pub_key));
