...
impl MerkleTreeWithHistory {
    ...
    /// Insert `N` leaves (even count) by calling `insert_two_leaves` `N/2` times.
    ///
    /// INVARIANT: `leaves.len()` must be even and `next_index` must be even at
    /// call time. Payroll batches of exactly 8 outputs always satisfy this, and
    /// inserting in pairs preserves the even-index invariant across calls.
    ///
    /// # Returns
    ///
    /// Returns the indexes where the leaves were inserted, in order.
    pub fn insert_n_leaves(env: &Env, leaves: Vec<U256>) -> Result<Vec<u32>, Error> {
        let n = leaves.len();
        ...
        let mut i: u32 = 0;
        while i < n {
            let leaf_1 = leaves.get(i).ok_or(Error::NotInitialized)?;
            let leaf_2 = leaves
                .get(i.checked_add(1).ok_or(Error::Overflow)?)
                .ok_or(Error::NotInitialized)?;
            let (idx_0, idx_1) = Self::insert_two_leaves(env, leaf_1, leaf_2)?;
